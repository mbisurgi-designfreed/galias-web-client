...
class UnidadVentaForm extends Component {
    DEFECTO = [{
        value: true,
        label: 'Si'
    }, {
    ...
    render() {
        return (
            <div>
                <h3 className="modal-title" >Unidad de Venta</h3>
                <Form className="form">
                    <div className="row">
                    ...
                    <button className="btn">{this.props.unidadVta.item ? 'Editar' : 'Agregar'}</button>
                </Form>
            </div>
        )
    }
...
const mapPropsToValues = ({ unidadVta }) => ({
    unidad: unidadVta.item ? unidadVta.item.unidad : '',
    equivalencia: unidadVta.item ? unidadVta.item.equivalencia : 0,
    defecto: unidadVta.item ? unidadVta.item.defecto : false,
});
...
const onSubmit = (values, { props, resetForm }) => {
    ...
    resetForm();
    props.onSubmit({ unidadVta, index: props.unidadVta.item ? props.unidadVta.index : null });
}
